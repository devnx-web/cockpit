...
function isSafeAccountLabel(label) {
  return typeof label === "string"
    && label.length > 0
    && label.length <= 200
    && label !== "."
    && label !== ".."
    && path.basename(label) === label
    && !label.includes("\0");
}

function managedCodexLabel(state, label) {
  if (!isSafeAccountLabel(label) || !Object.hasOwn(state?.accounts || {}, label)) {
    throw new Error(`conta Codex não encontrada: ${label}`);
  }
  return label;
}

function claudeBackupPath(dir, prefix, num, email) {
  const accountNumber = String(num);
  if (!/^\d+$/.test(accountNumber)) throw new Error("registro de conta Claude inválido");
  const safeEmail = String(email || "")
    .replace(/[\\/\0]/g, "_")
    .slice(0, 240);
  if (!safeEmail) throw new Error("e-mail da conta Claude inválido");
  return path.join(dir, `.${prefix}-${accountNumber}-${safeEmail}.json`);
}

function claudeCredentialPath(num, email) {
  return claudeBackupPath(CLAUDE_CRED_DIR, "claude-credentials", num, email);
}

function claudeAccountConfigPath(num, email) {
  return claudeBackupPath(CLAUDE_CONFIGS_DIR, "claude-config", num, email);
}
...
// O arquivo do Claude também pode conter tokens MCP (Figma e outros plugins).
// O gerenciador de contas deve transportar somente a sessão do Claude Code.
function claudeCredentialEnvelope(cred) {
  if (!cred?.claudeAiOauth) return null;
  return { claudeAiOauth: structuredClone(cred.claudeAiOauth) };
}
...
// Para a conta ativa, refresh fica a cargo do próprio Claude Code. A consulta
// de uso nunca rotaciona o refresh token por fora do processo da CLI.
// Retorna { status: "ok"|"expired"|"ratelimited"|"error", usage }.
async function claudeUsageWithRefresh(cred, credPath, allowRefresh = true) {
  const oauth = cred?.claudeAiOauth;
  if (!oauth?.accessToken) return { status: "expired" };
  let token = oauth.accessToken;

  // refresh proativo se o token já expirou (ou está perto)
  if (allowRefresh && oauth.expiresAt && oauth.expiresAt < Date.now() + 60000 && oauth.refreshToken) {
  ...
  // A conta ativa pode ter um refresh token perfeitamente válido, mas somente
  // a CLI deve rotacioná-lo. Não induza o usuário a relogar antes dessa tentativa.
  if (r.status === "unauthorized" && !allowRefresh && oauth.refreshToken) {
    return { status: "refresh_pending" };
  }

  // 401/403 -> token inválido: tenta renovar via refreshToken
  if (allowRefresh && r.status === "unauthorized" && oauth.refreshToken) {
    const nt = await claudeRefreshToken(cred, credPath);
    if (nt) {
...
async function claudeAccounts(force = false, cache = readCache()) {
  ...
  const out = await Promise.all(
    entries.map(async ([num, info]) => {
      ...
      const credPath = isActive
        ? CLAUDE_ACTIVE_CRED
        : claudeCredentialPath(num, email);
      ...
      const r = await claudeUsageWithRefresh(cred, credPath, !isActive);
      const usage = r.usage;
      ...
      if (["ratelimited", "error", "refresh_pending"].includes(r.status) && cached?.data) {
...
async function codexAccounts(force = false, cache = readCache()) {
...
export function codexSwitch(label) {
  ...
  managedCodexLabel(state, label);
  if (state.active === label) {
    return { ok: true, platform: "codex", active: label, unchanged: true };
  }
  const target = path.join(CODEX_STORE, `${label}.json`);
  const targetAuth = readJson(target);
  if (!targetAuth?.tokens) throw new Error(`backup Codex inválido: ${label}`);

  // preserva o token atual da conta ativa antes de trocar (pode ter renovado)
  if (state.active && isSafeAccountLabel(state.active) && Object.hasOwn(state.accounts, state.active)) {
    const currentAuth = readJson(CODEX_AUTH);
    if (!currentAuth?.tokens) {
      throw new Error("não foi possível preservar a sessão Codex ativa");
    }
    writeJson(path.join(CODEX_STORE, `${state.active}.json`), currentAuth, 0o600);
  }
  writeJson(CODEX_AUTH, targetAuth, 0o600);
  state.active = label;
...
export function claudeSwitch(email) {
  ...
  if (curEntry) {
    ...
    const currentCred = claudeCredentialEnvelope(readJson(CLAUDE_ACTIVE_CRED));
    if (!currentCred) throw new Error("não foi possível preservar a sessão Claude ativa");
    writeJson(
      claudeCredentialPath(curNum, curEmail),
      currentCred,
      0o600,
    );
    const curCfg = readJson(cfgPath);
    if (!curCfg?.oauthAccount) throw new Error("configuração da conta Claude ativa inválida");
    writeJson(
      claudeAccountConfigPath(curNum, curEmail),
      { oauthAccount: structuredClone(curCfg.oauthAccount) },
    );
  }

  // 2) lê os backups da conta alvo
  const tCred = readJson(claudeCredentialPath(num, email));
  const tCfg = readJson(claudeAccountConfigPath(num, email));
  if (!tCred?.claudeAiOauth) throw new Error(`backup de credenciais inválido para ${email}`);
  ...
  const activeCred = readJson(CLAUDE_ACTIVE_CRED) || {};
  activeCred.claudeAiOauth = structuredClone(tCred.claudeAiOauth);
  writeJson(CLAUDE_ACTIVE_CRED, activeCred, 0o600);
...
export function codexAdd() {
  ...
  label = label.replace(/[\\/\0]/g, "_").slice(0, 200);
...
export function claudeAdd() {
  ...
  const accountCred = claudeCredentialEnvelope(creds);
  if (!accountCred) throw new Error("sessão Claude Code ausente nas credenciais ativas");
  writeJson(claudeCredentialPath(num, email), accountCred, 0o600);
  writeJson(
    claudeAccountConfigPath(num, email),
    { oauthAccount: structuredClone(cfg.oauthAccount) },
  );
...
export function claudeRemove(email) {
  ...
  const [num] = entry;
  try { fs.unlinkSync(claudeCredentialPath(num, email)); } catch {}
  try { fs.unlinkSync(claudeAccountConfigPath(num, email)); } catch {}
...
async function loadUsageTable(force = false) {
  const cache = readCache();
  const [claude, codex] = await Promise.all([
    claudeAccounts(force, cache),
    codexAccounts(force, cache),
  ]);
  writeCache(cache);
  return {
...
export function getUsageTable(force = false) {
  // Uma única leitura/refresh por processo: evita clobber do cache e, mais
  // importante, duas rotações concorrentes do mesmo refresh token Claude.
  if (usageInFlight) return usageInFlight;
  usageInFlight = loadUsageTable(force).finally(() => { usageInFlight = null; });
