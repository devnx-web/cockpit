...
// O `agy` (Antigravity CLI) lê a sessão de um caminho fixo sob o HOME e não
// aceita variável de ambiente para redirecioná-lo. O isolamento por conta é
// feito trocando o HOME do processo do CLI — nunca o do terminal inteiro.
const GEMINI_TOKEN_RELATIVE_PATH = path.join(".gemini", "antigravity-cli", "antigravity-oauth-token");
const GEMINI_SHIM_NAME = process.platform === "win32" ? "agy.cmd" : "agy";
// Preferências do usuário podem ser compartilhadas; `antigravity-cli/` nunca,
// é onde mora a sessão e um symlink ali misturaria a conta do pool com o
// login pessoal nas duas direções.
const GEMINI_STATIC_DIRS = Object.freeze(["extensions", "commands"]);
const GEMINI_SNAPSHOT_FILES = Object.freeze(["settings.json", "GEMINI.md"]);
// Endereço deliberadamente inexistente: desliga o Secret Service apenas para o
// processo do CLI (veja o comentário em #installGeminiShim).
const GEMINI_BLOCKED_BUS = "unix:path=/nonexistent/cockpit-sem-keyring";
...
function normalizeProvider(provider) {
  ...
  if (!normalized) {
    throw new TeamAccountsError("Plataforma inválida; use openai, claude ou gemini.", {
...
function safeRelativePath(value, description) {
  const raw = requireString(value, description);
  const unsafe = path.isAbsolute(raw)
    || /^[a-zA-Z]:/.test(raw)
    || path.normalize(raw).split(/[\\/]+/).some((segment) => segment === "..");
  if (unsafe) {
    throw new TeamAccountsError(`O servidor enviou um ${description} fora do diretório do Cockpit.`, {
      code: "UNSAFE_SERVER_RESPONSE",
    });
  }
...
function normalizeGeminiClient(client) {
  if (!client || typeof client !== "object") {
    throw new TeamAccountsError("Resposta sem material de autenticação Google.", {
      code: "INVALID_SERVER_RESPONSE",
    });
  }
  if (typeof client.refresh_token === "string" && client.refresh_token.trim()) {
    throw new TeamAccountsError("O servidor tentou fornecer um refresh token Google não brokerado.", {
      code: "UNSAFE_SERVER_RESPONSE",
    });
  }
  const payload = requireString(client.payload, "sessão do Antigravity");
  let parsed;
  try {
    parsed = JSON.parse(payload);
  } catch {
    throw new TeamAccountsError("Sessão do Antigravity em formato inválido.", {
      code: "INVALID_SERVER_RESPONSE",
    });
  }
  const token = parsed?.token;
  if (!token || typeof token !== "object") {
    throw new TeamAccountsError("Sessão do Antigravity sem bloco de token.", {
      code: "INVALID_SERVER_RESPONSE",
    });
  }
  requireString(token.access_token, "access token Google");
  // Renovar é papel do painel, como nos outros dois provedores. Um refresh
  // token preenchido aqui significa que a política mudou do lado errado.
  ...
  return {
    type: client.type || "antigravity_oauth_token",
    path: safeRelativePath(client.path || GEMINI_TOKEN_RELATIVE_PATH, "caminho de sessão"),
    payload,
    expires_at: isoFromUnknown(client.expires_at),
  };
}
...
// Capability para consumidores que não são PTYs criados pelo Cockpit (LifeAi).
// O prefixo "cc-" não é cosmético: o SDK Anthropic só manda Authorization:
// Bearer quando o token parece OAuth, e é isso que o proxy lê. Com outro
// prefixo o cliente manda x-api-key e o broker responde 401.
function makeClaudeLeaseCapability() {
  return `cc-cockpit-${crypto.randomBytes(24).toString("base64url")}`;
}

const CLAUDE_LEASE_TTL_MS = 12 * 60 * 60 * 1000;
...
export class TeamAccountsClient {
  ...
  constructor({
    ...
    brokerGeminiHome = env.COCKPIT_TEAM_GEMINI_HOME || path.join(homeDir, ".cockpit", "gemini"),
    sourceGeminiHome = path.join(homeDir, ".gemini"),
    geminiCliPath = null,
    paths = {},
  } = {}) {
    ...
    this.#brokerGeminiHome = path.resolve(brokerGeminiHome);
    this.#unavailableGeminiHome = path.join(this.#brokerGeminiHome, "unavailable");
    this.#sourceGeminiHome = path.resolve(sourceGeminiHome);
    this.#geminiCliPath = geminiCliPath ? path.resolve(geminiCliPath) : null;
  ...
  #rememberSelection(provider, payload) {
    ...
    if (provider === "gemini") {
      const client = normalizeGeminiClient(payload?.client);
      // Mesma chave do Codex: cada conta ganha um HOME próprio, duas contas
      // convivem na máquina e um PTY antigo não muda de conta sozinho.
      const accountKey = crypto.createHash("sha256").update(accountId).digest("hex").slice(0, 32);
      const geminiHome = path.join(this.#brokerGeminiHome, "accounts", accountKey);
      this.#selected.set(provider, {
        account,
        client,
        geminiHome,
        shimDir: path.join(this.#brokerGeminiHome, "bin", accountKey),
        tokenPath: path.join(geminiHome, client.path),
      });
      return { account, clientType: client.type };
    }
  ...
  /**
   * Emite uma capability de vida limitada para um consumidor que não é um PTY
   * criado pelo Cockpit. O token vale só para o proxy Claude loopback e some
   * quando o processo do servidor cai, como a capability dos terminais.
   */
  issueClaudeCapability(label, ttlMs = CLAUDE_LEASE_TTL_MS) {
    const capability = makeClaudeLeaseCapability();
    this.#claudeLeases.set(capability, {
      label: String(label || "desconhecido"),
      expiresAt: Date.now() + Math.max(60_000, Number(ttlMs) || CLAUDE_LEASE_TTL_MS),
    });
    return capability;
  }

  /**
   * Empurra o vencimento de um lease vivo. O consumidor recebeu a capability no
   * env quando nasceu e não tem como reler: trocar o segredo o deixaria mudo.
   * Renovar o mesmo segredo mantém o TTL curto sem derrubar quem está usando —
   * e um lease órfão (o dono morreu, ninguém renova) vence sozinho.
   *
   * Devolve false se o lease não existe ou já venceu, para o chamador reemitir.
   */
  renewClaudeCapability(capability, ttlMs = CLAUDE_LEASE_TTL_MS) {
    for (const [lease, meta] of this.#claudeLeases) {
      if (!secretsEqual(capability, lease)) continue;
      if (meta.expiresAt <= Date.now()) {
        this.#claudeLeases.delete(lease);
        return false;
      }
      meta.expiresAt = Date.now() + Math.max(60_000, Number(ttlMs) || CLAUDE_LEASE_TTL_MS);
      return true;
    }
    return false;
  }

  revokeClaudeCapability(capability) {
    return this.#claudeLeases.delete(capability);
  }

  #claudeCapabilityValid(capability) {
    if (secretsEqual(capability, this.#claudeProxyCapability)) return true;
    // Comparação em tempo constante contra cada lease: o Map é indexado pelo
    // próprio segredo, então um get() direto vazaria timing na comparação de
    // chave. São poucos leases (um por consumidor), o custo é irrelevante.
    for (const [lease, meta] of this.#claudeLeases) {
      if (!secretsEqual(capability, lease)) continue;
      if (meta.expiresAt <= Date.now()) {
        this.#claudeLeases.delete(lease);
  ...
  async handleClaudeProxyRequest({ authorization, method, requestPath, headers = {}, body } = {}) {
    ...
    if (!this.#claudeCapabilityValid(capability)) {
  ...
  async publishCurrent(provider, options = {}) {
    ...
    // O login local do Antigravity mora no Secret Service e no HOME pessoal;
    // publicá-lo daqui exigiria ler a credencial do usuário. Contas Google
    // entram no pool pelo próprio painel.
    if (normalized === "gemini") {
      throw new TeamAccountsError("Contas Google são cadastradas diretamente no DevNX Control.", {
        code: "UNSUPPORTED_PROVIDER_PUBLISH",
      });
    }
  ...
  enrichPtyEnv(baseEnv = {}, { codexRefreshUrl, claudeProxyUrl, claudeProjectPath } = {}) {
    ...
    this.#applyGeminiShim(env);
    return env;
  }

  // O shim é o único ponto de entrada do `agy` dentro do PTY: ele troca o HOME
  // apenas do processo do CLI, deixando o terminal e o ~/.gemini pessoal
  // intactos. Sem seleção central o PATH aponta para um HOME vazio — o CLI
  // pede login em vez de cair na conta pessoal do usuário.
  #applyGeminiShim(env) {
    const inherited = String(env.PATH || "")
      .split(path.delimiter)
      .filter((entry) => entry && !this.#isGeminiShimDir(entry));

    const selected = this.#selected.get("gemini");
    const home = selected?.geminiHome || this.#unavailableGeminiHome;
    const shimDir = selected?.shimDir || path.join(this.#brokerGeminiHome, "bin", "unavailable");
    if (!selected) {
      // Um PTY pode nascer durante uma queda do Control. Guarde o HOME
      // fail-closed para que a primeira seleção posterior promova esse shell
      // sem mexer no PATH (o ambiente do processo é imutável após o spawn).
      deleteFile(path.join(this.#unavailableGeminiHome, GEMINI_TOKEN_RELATIVE_PATH));
      this.#pendingGeminiHomes.add(this.#unavailableGeminiHome);
    }

    const installed = this.#installGeminiShim(home, shimDir);
    env.PATH = (installed ? [installed, ...inherited] : inherited).join(path.delimiter);
    return env;
  }

  #isGeminiShimDir(entry) {
    const binRoot = path.join(this.#brokerGeminiHome, "bin");
    const resolved = path.resolve(entry);
    return resolved === binRoot || resolved.startsWith(`${binRoot}${path.sep}`);
  }

  // Resolve o `agy` real no PATH do host, ignorando os próprios shims. Máquina
  // sem Antigravity instalado é caso normal na frota: sem CLI não há nada a
  // interceptar e o PATH do terminal segue sem o diretório do shim.
  #resolveGeminiCli() {
    if (this.#geminiCliPath) {
      try {
        fs.accessSync(this.#geminiCliPath, fs.constants.X_OK);
        return this.#geminiCliPath;
      } catch {
        return null;
      }
    }
    for (const entry of String(this.#env.PATH || "").split(path.delimiter)) {
      if (!entry || this.#isGeminiShimDir(entry)) continue;
      const candidate = path.join(path.resolve(entry), GEMINI_SHIM_NAME);
      try {
  ...
  #installGeminiShim(home, shimDir) {
    const cli = this.#resolveGeminiCli();
    if (!cli) return null;
    this.#prepareGeminiHome(home);
    secureDirectory(shimDir);
    const quote = (value) => `'${String(value).replace(/'/g, "'\\''")}'`;
    // Medido no CLI 1.2.3: quando o Secret Service responde, o `agy` usa a
    // sessão do keyring e ignora o arquivo — trocar só o HOME autenticaria na
    // conta pessoal do usuário. Apontar o barramento para um caminho
    // inexistente faz o CLI cair no arquivo da conta do pool. Só o processo do
    // `agy` perde o Secret Service; o terminal continua com o dele.
    const shim = `#!/bin/sh\n`
      + `# Gerado pelo Cockpit. Executa o Antigravity CLI na conta do pool sem\n`
      + `# tocar no login pessoal em ~/.gemini nem no keyring do usuário.\n`
      + `exec env HOME=${quote(home)} DBUS_SESSION_BUS_ADDRESS=${quote(GEMINI_BLOCKED_BUS)} ${quote(cli)} "$@"\n`;
    const shimPath = path.join(shimDir, GEMINI_SHIM_NAME);
    atomicWriteFile(shimPath, shim);
    try { fs.chmodSync(shimPath, 0o700); } catch {}
    return shimDir;
  }

  #prepareGeminiHome(home) {
    secureDirectory(home);
    const destination = path.join(home, ".gemini");
    secureDirectory(destination);

    // Snapshot das preferências, nunca da autenticação local. Copiar em vez de
    // linkar evita que uma mudança global altere um terminal já aberto.
    for (const name of GEMINI_SNAPSHOT_FILES) {
      const source = path.join(this.#sourceGeminiHome, name);
      try {
        if (!fs.statSync(source).isFile()) continue;
        atomicWriteFile(path.join(destination, name), fs.readFileSync(source));
      } catch {}
    }

    for (const name of GEMINI_STATIC_DIRS) {
      const source = path.join(this.#sourceGeminiHome, name);
  ...
  // Escreve a sessão Google no HOME da conta. Chamada a cada bootstrap: quando
  // o painel renova o access token, o arquivo é reescrito no mesmo caminho.
  materializeGeminiSession() {
    const selected = this.#selected.get("gemini");
    if (!selected?.client?.payload) {
      throw new TeamAccountsError("Nenhuma conta Google foi selecionada.", {
        code: "NO_GEMINI_SELECTION",
      });
    }
    this.#prepareGeminiHome(selected.geminiHome);
    this.#writeGeminiToken(selected.tokenPath, selected.client.payload);

    // Shells criados enquanto a seleção central estava indisponível continuam
    // apontados para o HOME fail-closed; materialize a conta recuperada neles
    // uma vez e mantenha-os fixos nela, como acontece com o Codex.
    for (const pendingHome of [...this.#pendingGeminiHomes]) {
      this.#prepareGeminiHome(pendingHome);
      this.#writeGeminiToken(path.join(pendingHome, selected.client.path), selected.client.payload);
      this.#pendingGeminiHomes.delete(pendingHome);
    }
    return { path: selected.tokenPath, account: safeAccount(selected.account) };
  }

  #writeGeminiToken(tokenPath, payload) {
    const destination = path.resolve(tokenPath);
    const relative = path.relative(this.#brokerGeminiHome, destination);
    if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
      throw new TeamAccountsError("Sessão brokerada deve permanecer no diretório Gemini do Cockpit.", {
        code: "UNSAFE_GEMINI_TOKEN_PATH",
      });
    }
    secureDirectory(path.dirname(destination));
    atomicWriteFile(destination, payload);
  ...
  #clearBrokerFiles({ all = false } = {}) {
    ...
    for (const pendingHome of this.#pendingGeminiHomes) {
      try { deleteFile(path.join(pendingHome, GEMINI_TOKEN_RELATIVE_PATH)); } catch {}
    }
    this.#pendingGeminiHomes.clear();
    if (all && this.#brokerGeminiHome.startsWith(`${path.join(this.#homeDir, ".cockpit")}${path.sep}`)) {
      // Os shims só fazem sentido enquanto há seleção central; removê-los tira
      // o `agy` do PATH dos próximos terminais em vez de deixá-lo apontando
      // para um HOME sem sessão.
      try { fs.rmSync(path.join(this.#brokerGeminiHome, "bin"), { recursive: true, force: true }); } catch {}
    }
