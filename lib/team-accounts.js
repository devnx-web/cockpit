...
const CODEX_STATIC_DIRS = Object.freeze(["skills", "rules", "plugins", "packages", "agents", "hooks"]);
...
export class TeamAccountsClient {
  ...
  async #refreshOpenAiPayload(accountId) {
    ...
    for (const entry of this.#openAiCapabilities.values()) {
      if (entry.client.account_uuid === resolvedId) {
        ...
        if (this.#ownedAuthPaths.has(entry.authPath)) this.#writeCodexEntry(entry, entry.authPath);
      }
    }
  ...
  #prepareClaudeHome(projectPath) {
    ...
    // dialog even though the flag was intentionally supplied.
  ...
  enrichPtyEnv(baseEnv = {}, { codexRefreshUrl, claudeProjectPath } = {}) {
    const env = { ...baseEnv };
    ...
    deleteFile(path.join(this.#brokerClaudeHome, ".credentials.json"));
    deleteFile(path.join(this.#unavailableCodexHome, "auth.json"));
    ...
    const claude = this.#selected.get("claude")?.client;
    if (claude?.oauth_token) env.CLAUDE_CODE_OAUTH_TOKEN = claude.oauth_token;
