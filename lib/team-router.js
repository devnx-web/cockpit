...
// Gemini é opcional por desenho: uma frota sem conta Google livre não pode
// ficar sem terminal, e o bootstrap não deve repetir por causa dele.
const REQUIRED_PROVIDERS = ["openai", "claude"];
const JSON_HEADERS = {
...
export class TeamRouter {
  ...
  async #bootstrap({ syncUsage = false } = {}) {
    ...
    const settled = await Promise.allSettled(
      [...PROVIDERS].map(async (provider) => {
        ...
        if (provider === "gemini") this.client.materializeGeminiSession();
  ...
  async refreshSelectionsIfStale({
    ...
    const hasEveryProvider = () => {
      ...
      return REQUIRED_PROVIDERS.every((provider) => selected[provider]);
    };
    if (
      ...
      // Mesma razão para a sessão Google: outro processo do Cockpit pode ter
      // limpado o arquivo compartilhado antes deste PTY nascer.
      if (this.status().selected.gemini) {
        try { this.client.materializeGeminiSession(); }
        catch (error) { this.log.warn?.(`[cockpit/team] ${publicError(error).message}`); }
      }
    ...
    if (this.status().selected.gemini && !result.selections?.some((item) => item.provider === "gemini")) {
      try { this.client.materializeGeminiSession(); }
      catch (error) { this.log.warn?.(`[cockpit/team] ${publicError(error).message}`); }
    }
    return result;
  ...
  /**
   * Credenciais para um consumidor externo do proxy Claude (a LifeAi). O
   * sufixo /anthropic é o que faz o SDK aceitar um base_url loopback; sem ele
   * o cliente volta a falar direto com api.anthropic.com sem avisar.
   */
  issueClaudeAccess(label, ttlMs) {
    const baseUrl = this.brokerUrl();
    if (!baseUrl) return null;
    return {
      baseUrl: `${baseUrl}/team/claude/anthropic`,
      capability: this.client.issueClaudeCapability(label, ttlMs),
    };
  }

  /**
   * Mantém viva a sessão de um consumidor externo: revalida a seleção de conta
   * no DevNX Control (é ela que o proxy usa a cada requisição) e empurra o
   * vencimento do lease. Devolve false quando o lease já morreu — aí o chamador
   * precisa reemitir, o que na prática significa reiniciar o consumidor.
   */
  async renewClaudeAccess(capability, { ttlMs, syncUsage = false } = {}) {
    if (this.status().connected) {
      await this.refreshSelectionsIfStale({ maxAgeMs: 0, syncUsage });
    }
    return this.client.renewClaudeCapability(capability, ttlMs);
  }

  revokeClaudeAccess(capability) {
    return this.client.revokeClaudeCapability(capability);
  }

  async usageTable() {
    const accounts = await this.accounts();
    const platforms = { openai: "codex", claude: "claude", gemini: "gemini" };
    const table = { claude: [], codex: [], gemini: [], central: true };
    for (const account of accounts) {
      const platform = platforms[account.provider];
  ...
  async proxyClaude(req, res, parsedUrl) {
    ...
    // Clientes que só aceitam um endpoint Anthropic "plausível" (o Hermes/LifeAi
    // exige que a base_url termine em /anthropic, senão volta silenciosamente
    // para api.anthropic.com) apontam para /team/claude/anthropic. O sufixo é
    // decorativo: some aqui e o broker segue recebendo /v1/... como sempre.
    const rest = parsedUrl.pathname.slice("/team/claude".length);
    const stripped = rest.startsWith("/anthropic/") ? rest.slice("/anthropic".length)
      : rest === "/anthropic" ? "" : rest;
    const requestPath = `${stripped}${parsedUrl.search || ""}`;
