...
export class TeamRouter {
  ...
  assertLocalRequest(req, { sensitive = false } = {}) {
    ...
    if (sensitive && (req.headers.origin || req.headers.referer)) {
      throw httpError(403, "origem não permitida para renovação Codex");
    }
  }
  ...
  enrichPtyEnv(env, { claudeProjectPath } = {}) {
    ...
    return this.client.enrichPtyEnv(env, { codexRefreshUrl, claudeProjectPath });
  }
  ...
  async dispatch(req, parsedUrl) {
    ...
    if (route === "/team/optimize") {
      return { payload: await this.bootstrap({ syncUsage: body.syncUsage === true }) };
    }
