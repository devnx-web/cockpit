...
export function constantTimeTokenEqual(expected, provided) {
...
function safeTerminal(terminal) {
  return {
    ...
    // Endereço estável da frente: `id` é reciclado entre sessões do Cockpit, e
    // um relatório de ontem que fala em "t1" mente hoje.
    frontUid: terminal.frontUid ? String(terminal.frontUid) : null,
...
export function createControlApi({
   ...
   *
   * Antes disto, criar terminal e despachar agente funcionavam em qualquer
   * projeto do catálogo, com ou sem janela aberta nele: o servidor mantém uma
   * sessão para cada projeto desde o boot, então o trabalho começava, rodava e
   * terminava sem aparecer em lugar nenhum. Foi assim que um agente publicou
   * num repositório que o dono nem tinha na tela.
   *
   * `revealProject` no adaptador é quem decide: ou traz o projeto para a frente
   * na janela, ou recusa a chamada dizendo que não há janela para trazer. O
   * método é opcional — adaptador que não o implementa (testes, embarcados sem
   * interface) segue como antes, porque o gate é da janela, não da API.
   */
  async function exigirJanela(projectId, motivo, terminalId = null) {
    if (typeof adapter.revealProject !== "function") return;
    await adapter.revealProject(projectId, { reason: motivo, terminalId });
  }
  ...
  async function dispatch(req, parsedUrl) {
    ...
    if (
      ...
      return executeIdempotent(req, pathnameKey, body, async () => {
        requireProject(projectId);
        await exigirJanela(projectId, "create");
    ...
    if (segments.length === 3 && segments[2] === "dispatch") {
      ...
      return executeIdempotent(req, pathnameKey, body, async () => {
        ...
        // Antes de abrir a demanda: sem janela não há despacho. Recusar aqui, e
        // não lá dentro do dispatcher, evita deixar uma demanda natimorta na
        // lista só para dizer o que já dava para dizer na resposta.
        await exigirJanela(project.id, "dispatch");
    ...
    if (
      ...
      return executeIdempotent(req, pathnameKey, body, async () => {
        ...
        await exigirJanela(projectId, "input", terminal.id);
