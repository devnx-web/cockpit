...
const CAPABILITIES = new Set(["read", "create", "input", "interrupt"]);
...
export function normalizeControlPolicy(
  ...
  const available = new Set(availableProjectIds.map(String));
  const projects = new Set(
    requestedProjects
      .map(String)
      .filter((id) => /^[a-z0-9_-]+$/.test(id) && available.has(id)),
  );
...
export function loadControlPolicy(
  policyPath,
  { availableProjectIds = [], log = console } = {},
) {
  if (!policyPath) {
    return normalizeControlPolicy({}, { availableProjectIds });
  }
  try {
    const parsed = JSON.parse(fs.readFileSync(policyPath, "utf8"));
    return normalizeControlPolicy(parsed, { availableProjectIds });
  } catch (error) {
...
function defaultRuntimeDir() {
  ...
  if (base && path.isAbsolute(base)) return path.join(base, "cockpit");
  const stateHome = process.env.XDG_STATE_HOME;
  if (stateHome && path.isAbsolute(stateHome)) {
    return path.join(stateHome, "cockpit");
  }
  const home = os.homedir();
  if (home && path.isAbsolute(home)) {
    return path.join(home, ".local", "state", "cockpit");
  }
  const uid =
    typeof process.getuid === "function" ? process.getuid() : os.userInfo().username;
  return path.join(os.tmpdir(), `cockpit-${uid}`);
}
...
export function createControlApi({
  ...
  if (!policy?.projects || !policy?.capabilities) {
    throw new TypeError("política de controle normalizada é obrigatória");
  }
  const idempotency = new Map();

  function requireCapability(capability) {
    if (!policy.enabled || !policy.capabilities.has(capability)) {
      throw new ControlHttpError(
        403,
        "PROJECT_FORBIDDEN",
  ...
  function requireProject(projectId) {
    if (!policy.enabled || !policy.projects.has(projectId)) {
      throw new ControlHttpError(
        403,
        "PROJECT_FORBIDDEN",
  ...
  function requireTerminal(projectId, terminalId) {
    ...
    if (!terminal || !terminalAllowed(policy, terminal)) {
      throw new ControlHttpError(
        404,
        "TERMINAL_NOT_FOUND",
  ...
  async function dispatch(req, parsedUrl) {
    ...
    if (
      ...
      const projects = adapter
        .listProjects()
        .filter((project) => policy.projects.has(project.id))
        .map(safeProject);
      return {
        statusCode: 200,
        payload: {
    ...
    if (
      ...
      const terminals = adapter
        ...
        .filter((terminal) => terminalAllowed(policy, terminal))
        .map(safeTerminal);
      return {
        statusCode: 200,
        payload: {
