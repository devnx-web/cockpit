...
export function createLifeAiClient({ env = process.env, log = console } = {}) {
  ...
  /**
   * Aviso de "o agente parou". Lança `LifeAiUnavailable` com o serviço fora do
   * ar, como os demais — quem chama é a ponte, que trata desligada como
   * silêncio e deixa o cron de segurança cobrir.
   */
  async function cronWake(payload) {
    return require_().cronWake(payload);
  }

  return { state, ask, approve, stopRun, cronWake, consoleUrl, descriptorPath };
}
