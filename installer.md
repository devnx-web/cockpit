...
Versão atual: **0.22.0** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.22.0/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.22.0/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.22.0/cockpit-devnx_0.22.0_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.22.0/Cockpit-0.22.0.AppImage -o ~/Cockpit.AppImage
...
# espera: ii  cockpit-devnx  0.22.0  amd64
...
```bash
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.22.0/install.sh | sudo bash
```
...
Disponíveis no mesmo bucket — basta trocar `v0.22.0` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.22.0
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.22.0    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.22.0    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.22.0/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.22.0/install.sh | sudo bash
```

Commitar a v0.22.0 (tag opcional):

```bash
git add -A && git commit -m "release: v0.22.0 — <resumo>"
git tag v0.22.0
git push --tags
```
