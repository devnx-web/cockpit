...
Versão atual: **0.16.1** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.16.1/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.16.1/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.16.1/cockpit-devnx_0.16.1_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.16.1/Cockpit-0.16.1.AppImage -o ~/Cockpit.AppImage
...
# .deb
dpkg -l cockpit
# espera: ii  cockpit-devnx  0.16.1  amd64
...
Disponíveis no mesmo bucket — basta trocar `v0.16.1` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.16.1
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.16.1    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.16.1    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.16.1/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.16.1/install.sh | sudo bash
```

Commitar a v0.16.1 (tag opcional):

```bash
git add -A && git commit -m "release: v0.16.1 — <resumo>"
git tag v0.16.1
git push --tags
```
