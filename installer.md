...
Versão atual: **0.19.1** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.19.1/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.19.1/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.19.1/cockpit-devnx_0.19.1_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.19.1/Cockpit-0.19.1.AppImage -o ~/Cockpit.AppImage
...
# espera: ii  cockpit-devnx  0.19.1  amd64
...
> ⚠️ O nome do pacote é **`cockpit-devnx`**, não `cockpit`. O pacote `cockpit` do Debian/Ubuntu é o Web Console do sistema (`cockpit-ws`, `cockpit-system`, …) e não tem relação com este app — remover ele por engano derruba o console web da máquina.

```bash
pkill -f "/opt/Cockpit/cockpit"       # fecha o app antes
sudo apt remove cockpit-devnx         # mantém a config do usuário
sudo apt purge cockpit-devnx          # remove tudo
rm -rf ~/.config/Cockpit              # opcional — apaga config/cache do usuário
```

Para reinstalar depois:
```bash
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.19.1/install.sh | sudo bash
```
...
Disponíveis no mesmo bucket — basta trocar `v0.19.1` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.19.1
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.19.1    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.19.1    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.19.1/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.19.1/install.sh | sudo bash
```

Commitar a v0.19.1 (tag opcional):

```bash
git add -A && git commit -m "release: v0.19.1 — <resumo>"
git tag v0.19.1
git push --tags
```
