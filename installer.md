...
Versão atual: **0.12.4** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.12.4/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.12.4/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.12.4/cockpit-devnx_0.12.4_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.12.4/Cockpit-0.12.4.AppImage -o ~/Cockpit.AppImage
...
# .deb
dpkg -l cockpit
# espera: ii  cockpit-devnx  0.12.4  amd64
...
Disponíveis no mesmo bucket — basta trocar `v0.12.4` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.12.4
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.12.4    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.12.4    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.12.4/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.12.4/install.sh | sudo bash
```

Commitar a v0.12.4 (tag opcional):

```bash
git add -A && git commit -m "release: v0.12.4 — <resumo>"
git tag v0.12.4
git push --tags
```
