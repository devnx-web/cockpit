...
Versão atual: **0.6.15** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.6.15/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.6.15/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.6.15/cockpit-devnx_0.6.15_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.6.15/Cockpit-0.6.15.AppImage -o ~/Cockpit.AppImage
...
# .deb
dpkg -l cockpit
# espera: ii  cockpit-devnx  0.6.15  amd64
...
Disponíveis no mesmo bucket — basta trocar `v0.6.15` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.6.15
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.6.15    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.6.15    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.6.15/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.6.15/install.sh | sudo bash
```

Commitar a v0.6.15 (tag opcional):

```bash
git add -A && git commit -m "release: v0.6.15 — <resumo>"
git tag v0.6.15
git push --tags
```
