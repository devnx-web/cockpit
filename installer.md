...
Versão atual: **0.8.0** ([changelog](https://arquivos.devnx.com.br/cockpit/v0.8.0/CHANGELOG.md))

> Instalador one-liner: `curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.8.0/install.sh | sudo bash`
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.8.0/cockpit-devnx_0.8.0_amd64.deb -o /tmp/cockpit.deb
...
```bash
curl -L https://arquivos.devnx.com.br/cockpit/v0.8.0/Cockpit-0.8.0.AppImage -o ~/Cockpit.AppImage
...
# .deb
dpkg -l cockpit
# espera: ii  cockpit-devnx  0.8.0  amd64
...
Disponíveis no mesmo bucket — basta trocar `v0.8.0` no path:
...
```bash
COCKPIT_AWS_PROFILE=outro-profile  npm run release -- 0.8.0
COCKPIT_SKIP_UPLOAD=1              npm run release -- 0.8.0    # só local
COCKPIT_SKIP_BUILD=1               npm run release -- 0.8.0    # só upload
```
...
```bash
curl -sI https://arquivos.devnx.com.br/cockpit/v0.8.0/install.sh   # HTTP/2 200
curl -fsSL https://arquivos.devnx.com.br/cockpit/v0.8.0/install.sh | sudo bash
```

Commitar a v0.8.0 (tag opcional):

```bash
git add -A && git commit -m "release: v0.8.0 — <resumo>"
git tag v0.8.0
git push --tags
```
