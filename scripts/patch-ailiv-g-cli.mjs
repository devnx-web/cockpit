...
import { applyAilivGCliBranding, hasAilivGCliBranding } from "../lib/ailiv-g-cli-branding.js";

function resolveBinary() {
  const explicit = process.argv.find((arg) => arg.startsWith("--binary="))?.slice(9);
  const command = explicit || execFileSync("which", ["codex"], { encoding: "utf8" }).trim();
  return fs.realpathSync(command);
}

function atomicReplace(destination, contents, mode) {
  const temporary = `${destination}.ailiv-g-${process.pid}.tmp`;
  try {
...
const backup = `${binary}.before-ailiv-g`;
...
const result = applyAilivGCliBranding(original);
if (result.total === 0) {
  if (hasAilivGCliBranding(original)) {
    console.log(`A identidade Ailiv G já está aplicada em ${binary}`);
    process.exit(0);
  }
...
console.log(`Identidade Ailiv G aplicada em ${binary}`);
